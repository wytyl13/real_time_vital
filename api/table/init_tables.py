...
#                 # 检查设备是否已存在
#                 result = await session.execute(
#                     select(DeviceInfo).where(DeviceInfo.device_id == device_data["device_id"])
#                 )
...
#                     print(f"✓ 创建设备: {device_data['device_name']} ({device_data['device_id']})")
#                 else:
#                     print(f"- 设备已存在: {device_data['device_name']} ({device_data['device_id']})")
...
#                 result = await session.execute(
#                     select(SleepStatistics).where(
#                         SleepStatistics.device_sn == sleep_data["device_sn"],
#                         SleepStatistics.sleep_start_time == sleep_data["sleep_start_time"]
#                     )
#                 )
#                 existing_record = result.scalar_one_or_none()
                
#                 if not existing_record:
#                     new_record = SleepStatistics(**sleep_data)
